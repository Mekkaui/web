...
static void bomb_load() {
	bombs[4] = load_image(MAP_BOMB_TTL4);
	bombs[3] = load_image(MAP_BOMB_TTL3);
	bombs[2] = load_image(MAP_BOMB_TTL2);
	bombs[1] = load_image(MAP_BOMB_TTL1);
	bombs[0] = load_image(MAP_EXPLOSION);
}
...
SDL_Surface* sprite_get_bomb(short bomb_number) {
		assert(bomb_number >= 0 && bomb_number < 5);
		return bombs[bomb_number];
}
